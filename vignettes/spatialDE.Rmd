...
results <- spatialDE::run_spatialDE(X, sample_resid_expr)
head(results)
```
...
# ms_results <- spatialDE::run_model_search(X, resid_expr, de_results)
ms_results <- spatialDE::run_model_search(X, sample_resid_expr, de_results)
ms_results
```

### Plots
```{r}
svgs <- de_results[order(de_results$qval), ]$g
